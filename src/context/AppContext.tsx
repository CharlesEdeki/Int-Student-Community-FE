...
import type { UserDto, ApiResponse, CompleteProfileRequest, GroupDto, EventDto, MessageDto, NotificationDto, AnnouncementDto, AuthResponse, GroupMemberDto } from '@/services/api/types';
...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const createEvent = useCallback(async (eventData: Omit<Event, 'id' | 'organizer' | 'attendees'>) => {
    ...
    try {
      ...
      if (response.success && response.data) {
        const newEvent = mapDtoToEvent(response.data);
        newEvent.attendees = [state.currentUser.id];
    ...
    try {
      ...
      if (response.success && response.data) {
        const newPoll: Poll = {
          ...
          options: response.data.options.map(o => ({ id: o.id, text: o.text, votes: o.voteCount })),
          votedBy: [],
        };
        setState(prev => ({
          ...prev,
  ...
  const addUser = useCallback(async (userData: Omit<User, 'id'>) => {
    // Admin adding a user - in a real scenario this would be an admin API call
